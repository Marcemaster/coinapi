...
from criptovalue.vistas import CriptoValueView, Exchanger
from tkinter import *
...
class CriptoValueController():
    ...
    def ejecuta(self):
        ...
        while seguir == 's':
            ...
            self.modelo.obtener()
            self.vista.muestra(
                self.modelo.de, self.modelo.a, self.modelo.valor)
...
class Controller(Tk):
    def __init__(self):
        ...
        self.changer = Exchanger(self, self.probatinas)
        self.changer.pack(side=TOP)
        self.modelo = CriptoValueModel()

    def probatinas(self):
        print("Calcular cambio entre", self.changer.moneda_desde(),
              "-", self.changer.moneda_hasta())
