...
class Exchanger(ttk.Frame):
    def __init__(self, parent, funcion_click_del_controlador):
        ...
        self.desde = ttk.Combobox(
            self, values=MONEDAS, textvariable=self.desde_valor)
        self.desde.grid(column=0, row=0)

        self.hasta_valor = StringVar()
        self.hasta = ttk.Combobox(
            self, values=MONEDAS, textvariable=self.hasta_valor)
        ...
        self.calcular = ttk.Button(
            self, text="Calcular", command=funcion_click_del_controlador)
